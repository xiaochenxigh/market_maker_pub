        ...
        async with aiohttp.ClientSession(auth=auth) as session:
            if method == "GET":
                async with session.get(url, params=params, timeout=5) as response:
                    print(await response.read(),response.status)
...
class Loex:
    def __init__(self, api_key, secret_key):
        self.__url = 'https://openapi.loex.io'
    ...
    def _produce_sign(self, params):
        ...
        #return hashlib.md5().update(sign_str.encode('utf8')).hexdigest()
    ...
    async def depth_my(self, symbol):
        path = self.__url + '/open/api/new_order'
        request = {
            "pageSize": 200,
            "symbol": symbol
        }
